document.addEventListener("DOMContentLoaded", function () {
    ...
    addBeneficiaryForm.addEventListener("submit", async function (event) {
        ...
        let accountNumber = accountNumberElement.value;
        let accountName = accountNameElement.textContent;
        let nickname = nicknameElement.value;
        let bankCode = bankSelectElement.value;
        ...
        if (bankCode) bankCode = bankCode.trim();

        let bank = "";
        if (bankSelectElement.selectedIndex !== -1) {
            let selectedOption = bankSelectElement.options[bankSelectElement.selectedIndex];
            if (selectedOption && selectedOption.textContent) {
                bank = selectedOption.textContent.trim();
            }
        }
        ...
        try {
            ...
            document.getElementById("accountName").textContent = "";
            const response = await fetch("https://trustpoint.wuaze.com/User/asset/include/beneficiary.php", {
                method: "POST",
...
// ✅ Load Banks
function loadBanks() {
    fetch("https://trustpoint.wuaze.com/User/asset/include/fetch_banks.php")
...
async function fetchAccountName() {
    ...
    try {
        ...
        const primaryResponse = await fetch("./asset/include/fetch_account_name_bank.php", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (primaryData.success) {
            ...
            const backupResponse = await fetch("./asset/include/check_account_db.php", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({ accountNumber, bankCode }),
            });
...
function loadUserData() {
    fetch("https://trustpoint.wuaze.com/User/asset/include/beneficiary.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
