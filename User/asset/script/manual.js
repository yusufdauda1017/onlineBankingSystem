    ...
    accountInput.addEventListener("input", function () {
        ...
        debounceTimer = setTimeout(() => {
            if (accountNumber.length === 10) {
                ...
                fetch("./asset/include/bank_detect.php", {
                    method: "POST",
...
async function fetchPrimaryAccountName(accountNumber, bankCode) {
    const response = await fetch("./asset/include/fetch_account_name_bank.php", {
        method: "POST",
...
async function fetchBackupAccountName(accountNumber, bankCode) {
    const response = await fetch("./asset/include/check_account_db.php", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ accountNumber, bankCode }),
    });
...
async function loadBanks() {
    try {
        const response = await fetch("./asset/include/fetch_banks.php");
        const data = await response.json();
